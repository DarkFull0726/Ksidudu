...
def create_vmess_miami(email, days=3):
    ...
    vmess = {"v":"2","ps":"Miami-SSHFREE","add":"207.246.72.79","port":"443",
...
def api_create():
    ...
    if session["credits"] <= 0: return jsonify({"ok":False,"error":f"Sin creditos. Proximo en: {time_to_next(cid)}"}), 429
    ...
    spend_credit(cid); save_user(cid, ssh_user, vps_key); session_after = apply_regen(cid)
    ...
    return jsonify({"ok":True,"username":ssh_user,"password":password,"vps":vps_key,
        ...
        "ports":vps_info.get("PORTS","SSH:22  |  SSL:443  |  UDP:36712"),
